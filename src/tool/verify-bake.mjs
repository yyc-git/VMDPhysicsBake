...
import { sanitizeSjis, CharsetEncoder } from './vmd-writer.mjs';

const SCRIPT_DIR = path.dirname(fileURLToPath(import.meta.url));
const PROJECT_ROOT = path.resolve(SCRIPT_DIR, '../../');
const resolveFrom = (base, p) => (path.isAbsolute(p) ? p : path.resolve(base, p));

// MMM 参考烘焙（第三方物理烘焙产物，用于粗对比）。本仓库不携带该资产：
// 存在则做粗对比（informational，不判 FAIL），缺失则跳过并记录 skipped。
const MMM_DEFAULT = path.join(PROJECT_ROOT, 'output', 'pickup_bake_mmm_reference.vmd');
const PMX_DEFAULT = path.join(PROJECT_ROOT, 'demo', 'assets', 'Tda HMS illustrious Prom Dress Ver1.00 [Silver].pmx');
const VMD_RAW_DEFAULT = path.join(PROJECT_ROOT, 'demo', 'assets', 'pickup.vmd');
...
function main() {
  ...
  try {
    ...
    const mmmAvailable = fs.existsSync(MMM_PATH);
    const vmdMmm = mmmAvailable ? parser.parseVmd(readBuf(MMM_PATH), true) : null;
    ...
    if (mmmAvailable) console.log(`mmm motions=${vmdMmm.motions.length} distinctBones=${new Set(vmdMmm.motions.map(m => m.boneName)).size}`);
    else console.log(`mmm: ${MMM_PATH} 不存在 → 粗对比跳过`);
    ...
    const mmmByBone = vmdMmm ? groupByBone(vmdMmm.motions) : null;
    ...
    // ================= V6 可复跑确定性 =================
    {
      ...
      if (cli.noRebake) {
        ...
        try {
          const t0 = Date.now();
          const hookUrl = pathToFileURL(path.join(SCRIPT_DIR, 'register-hooks.mjs')).href;
          const cmd = `node --import ${JSON.stringify(hookUrl)} ${JSON.stringify(path.join(SCRIPT_DIR, 'bake-physics.mjs'))} --output ${JSON.stringify(tmpOut)}`;
    ...
    // ================= MMM 粗对比（仅参考，不判 FAIL）=================
    if (!mmmAvailable) {
      report.compareWithMmm = { skipped: true, note: 'MMM 参考不存在，粗对比跳过', mmmPath: MMM_PATH };
      console.log('[MMM] 参考缺失，跳过粗对比');
    } else {
