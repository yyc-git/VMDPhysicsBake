...
// ★ freq60 实验：物理按游戏 60fps 渲染帧步进（每 bake 帧调用 2 次 physics.update(1/60)），
// interval=3 → 物理 20Hz（游戏 60fps/3），与游戏物理频率一致（离线 30fps/3=10Hz 有 2× 差距）。
const physStepDt = dt / 2;
...
for (let frame = 0; frame <= maxFrame; frame++) {
  if (helperDriver) {
    ...
    // ★ freq60：与游戏完全同构的 60Hz 交错步进 — 每 bake 帧做 2 个 1/60 子步，
    // 每个子步：mixer.update(1/60) → IK → grant → physics.update(1/60)。
    // interval=3 → 物理在每 3 个 physics.update 调用（= 3/60 = 0.05s）时 step 一次 = 20Hz，
    // 与游戏「60fps 渲染帧 × interval=3 → 20Hz 物理」逐子步对齐（含 kinematic 目标在子步间推进）。
    for (let s = 0; s < 2; s++) {
      if (frame > 0) {
        mixer.update(physStepDt);   // 推进动画 1/60s（每 bake 帧 2 子步 = 1/30s，与 30fps 输出对齐）
      }
      mesh.updateMatrixWorld(true);
      ikSolver.update();
      grantSolver.update();
      mesh.updateMatrixWorld(true);
      physics.update(physStepDt);
    }
  }
  if (_DBG && frame % 10 === 0) {
