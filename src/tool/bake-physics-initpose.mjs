...
// ---- Ammo 全局注入 ----
// 仓库内 lib/ammo/ammo.wasm.js 是 CJS 工厂函数，Node 里用 createRequire 加载，
...
if (helperDriver) {
  ...
  helper.add(mesh, {
    ...
    warmup: physicsParams.warmupFrames,     // 对齐游戏 _setupMeshPhysics 内部 warmup
...
// ---- 5.5 初始位形注入（实验版）----
// 把游戏 warmupEndBones（warmup 60 帧后的真实骨位形快照）作为离线模拟的初始位形。
// 若游戏侧起点 == 注入位形 → 起点完美对齐，diverge 骨应消失。
// 注意坐标系：warmupEndBones 是 game 内 RIGHT 空间局部 quaternion（与 bake 内部一致），
// 直接赋值 bone.quaternion，禁止二次 LEFT/RIGHT 还原（parseVmd(ab,true) 同款 RIGHT 处理）。
const initPoseFile = config.initPoseFile;
const initPoseMesh = config.initPoseMesh || '$girl$_PlayerGoddess';
const initPoseEntry = config.initPoseEntry ?? 0;
let injectedCount = 0, missingCount = 0;
if (initPoseFile) {
  const initPath = path.isAbsolute(initPoseFile) ? initPoseFile : resolveFrom(SCRIPT_DIR, initPoseFile);
  const initRaw = JSON.parse(fs.readFileSync(initPath, 'utf8'));
  const weArr = Array.isArray(initRaw.warmupEndBones) ? initRaw.warmupEndBones
    : (Array.isArray(initRaw) ? initRaw : null);
  const entries = (weArr || []).filter(w => w && w.meshName === initPoseMesh);
  const entry = entries[initPoseEntry];
  if (!entry) {
    console.error(`[initpose] FATAL: ${initPoseFile} 无 ${initPoseMesh}#${initPoseEntry}（可用: ${[...new Set((weArr||[]).map(w=>w&&w.meshName))].join(', ')}）`);
    process.exit(1);
  }
  const poseBones = entry.bones || {};
  const poseNames = new Set(Object.keys(poseBones));
  // 覆盖统计：对比物理骨覆盖情况
  let covered = 0;
  for (const bn of physicsBoneNames) if (poseNames.has(bn)) covered++;
  console.log(`[initpose] warmupEndBones 覆盖物理骨: ${covered}/${physicsBoneNames.size}`);
  // 注入：只改物理骨（type1/2 关联骨），动作骨保持 mixer t=0 姿态
  for (const bi of physBoneIndices) {
    const bone = bones[bi];
    const q = poseBones[bone.name];
    if (!q || q.length !== 4) { missingCount++; continue; }
    bone.quaternion.set(q[0], q[1], q[2], q[3]);
    injectedCount++;
  }
  console.log(`[initpose] 注入 ${injectedCount} 物理骨（缺 ${missingCount}）`);
  // 刚体重置到注入后的骨骼位形（否则刚体仍停在 bind pose）
  mesh.updateMatrixWorld(true);
  if (physics && typeof physics.reset === 'function') {
    try { physics.reset(); } catch (e) { console.warn('[initpose] physics.reset failed:', e.message); }
  }
  // 清零速度（对齐 game _animateMesh(0)+reset 后、warmup 前的语义）
  try {
    const _zv = new Ammo.btVector3(0, 0, 0);
    for (const b of physics.bodies) { b.body.setLinearVelocity(_zv); b.body.setAngularVelocity(_zv); }
  } catch (e) {}
} else {
  console.log('[initpose] 未配置 initPoseFile，跳过注入（普通 bake）');
}
