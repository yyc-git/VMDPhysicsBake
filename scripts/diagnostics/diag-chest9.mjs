// 诊断17：官方帧循环顺序（先动画后物理）+ spring 原值 scale=1
// 对比官方顺序 vs 当前顺序，spring scale 1 vs 2000
...
const order = process.argv[3] ?? 'official'; // official=先动画后物理 | current=先物理后动画
...
console.log(`scale=${scale} order=${order}  左胸上: ${fmt(angs.left)}  右胸上: ${fmt(angs.right)}  前髪: ${fmt(angs.mae)}`);
