// 诊断11：胸部专项——spring damping 影响（0.05 vs 1.0 vs 默认不设）+ scale 1000/2000
...
const dampMode = process.argv[3] ?? 'default'; // default | 0.05 | 1.0
...
for (const c of ph.constraints) {
  ...
  } else if (dampMode === '1.0') {
    if (typeof cst.setDamping === 'function') { try { for (let i = 0; i < 6; i++) cst.setDamping(i, 1.0); } catch (e) {} }
  }
  // default: 不调用 setDamping（保持 Bullet 默认）
}
...
console.log(`scale=${scale} damp=${dampMode}  左胸上: ${fmt(angs.left)}  右胸上: ${fmt(angs.right)}  前髪: ${fmt(angs.mae)}`);
