// 诊断12：轻刚体 spring 强度专项——按刚体质量归一化 stiffness
// 假设：Bullet spring stiffness 应该与刚体质量匹配，轻刚体需要相对更强的 spring
// 参数：scale 全局 + lightScale（weight<0.1 的刚体用 scale/lightScale 即更强）
...
const scale = parseFloat(process.argv[2] ?? '2000');
const lightScale = parseFloat(process.argv[3] ?? '1'); // 1=与全局相同
...
// 按约束的刚体质量归一化 stiffness：轻刚体（weight<0.1）用 lightScale
const rbByIndex = pmx.rigidBodies;
{
  ...
  Ammo.btGeneric6DofSpringConstraint.prototype.setStiffness = function (idx, val) {
    // 无法从 constraint 反查刚体质量，改用全局 lightScale 不区分
...
// 轻刚体约束：单独重新设置 stiffness（更强的 spring）
for (const c of ph.constraints) {
  const cst = c.constraint;
  // 判断该约束是否连接轻刚体
  const rbA = pmx.rigidBodies[c.params.rigidBodyIndex1];
  const rbB = pmx.rigidBodies[c.params.rigidBodyIndex2];
  const light = (rbA && rbA.weight < 0.1) || (rbB && rbB.weight < 0.1);
  if (light) {
    const sp = c.params.springPosition, sr = c.params.springRotation;
    for (let i = 0; i < 3; i++) {
      if (sp && sp[i] !== 0) { try { cst.setStiffness(i, sp[i] / (scale / lightScale)); } catch (e) {} }
      if (sr && sr[i] !== 0) { try { cst.setStiffness(i + 3, sr[i] / (scale / lightScale)); } catch (e) {} }
    }
  }
  const sp = c.params.springPosition, sr = c.params.springRotation;
...
console.log(`scale=${scale} lightScale=${lightScale}  左胸上: ${fmt(angs.left)}  右胸上: ${fmt(angs.right)}  前髪: ${fmt(angs.mae)}`);
