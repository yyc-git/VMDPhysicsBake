// 诊断20：maxStepNum 细分——步进越多 solver 越能跟上 kinematic 快速移动
...
const maxStepNum = parseFloat(process.argv[2] ?? '3');
...
const pp = { unitStep: 1 / 65, gravity: [0, -98, 0], maxStepNum };
...
console.log(`maxStepNum=${maxStepNum} 平均角差=${avg.toFixed(1)}° 中位数=${median.toFixed(1)}° 翻转骨=${flips.length} 翻转总帧=${flipTotal}`);
