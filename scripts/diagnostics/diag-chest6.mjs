// 诊断14：轻刚体质量钳制——weight<0.1 的刚体强制 weight=1（模拟 MMD 原生 Bullet 可能的质量处理）
...
const minWeight = parseFloat(process.argv[2] ?? '1'); // 轻刚体钳制到的最小质量
...
const rigidBodyParams = pmx.rigidBodies.map((rb, i) => {
  ...
  // 质量钳制：type!=0 且 weight < minWeight → 提升
  if (p.type !== 0 && p.weight < minWeight) {
    p.weight = minWeight;
  }
  return p;
});
...
console.log(`minWeight=${minWeight}  左胸上: ${fmt(angs.left)}  右胸上: ${fmt(angs.right)}  前髪: ${fmt(angs.mae)}`);
