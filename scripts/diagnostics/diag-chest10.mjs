// 诊断18：spring vs nospring 全 163 物理骨对比——角差中位数 + 翻转帧数
...
const mode = process.argv[2] ?? 'spring'; // spring | nospring
...
if (mode === 'nospring') {
  ...
  Ammo.btGeneric6DofSpringConstraint.prototype.setStiffness = function (idx, val) { return this; };
  const _origEnable = Ammo.btGeneric6DofSpringConstraint.prototype.enableSpring;
  Ammo.btGeneric6DofSpringConstraint.prototype.enableSpring = function (idx, on) { return _origEnable.call(this, idx, false); };
} else {
  const _orig = Ammo.btGeneric6DofSpringConstraint.prototype.setStiffness;
...
for (const [nm, frames] of Object.entries(recorded)) {
  ...
  flipTotal += boneFlip;
  if (diffCount) angDiff.push(diffSum / diffCount);
}
...
console.log(`mode=${mode} 对比骨数=${angDiff.length} 平均角差=${avg.toFixed(1)}° 中位数=${median.toFixed(1)}° 翻转骨=${flips.length} 翻转总帧=${flipTotal}`);
console.log(' 翻转骨:', flips.slice(0, 20).join(', '));
