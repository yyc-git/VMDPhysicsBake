// diag-fix4-sweep.mjs — fix4 干净 stiffness scale 扫描（每次 run 还原 setStiffness 原型，杜绝叠加 bug）
// 目标：找到起身段（f38-50）前髪１ 全部 < 40° 的 springStiffnessScale，同时 err 尽量低。
...
const ORIG_SET_STIFFNESS = Ammo.btGeneric6DofSpringConstraint.prototype.setStiffness;
const MMM = { 38: 17, 39: 17, 40: 17, 41: 16, 42: 16, 43: 16, 44: 15, 45: 14, 46: 15, 47: 15, 48: 16, 49: 16, 50: 16 };

async function run(label, opts) {
  Ammo.btGeneric6DofSpringConstraint.prototype.setStiffness = ORIG_SET_STIFFNESS;
  const stiffScale = opts.stiffScale ?? 1;
  if (stiffScale !== 1) {
    Ammo.btGeneric6DofSpringConstraint.prototype.setStiffness = function(idx, val) { ORIG_SET_STIFFNESS.call(this, idx, val / stiffScale); };
  }
  ...
  const allUnder40 = vals.every(v => v !== 'NaN' && Number(v) < 40);
  console.log(label.padEnd(42), vals.join(' '), ' err:', err.toFixed(0), allUnder40 ? ' [OK all<40]' : '');
}

console.log('起身段(38-50)前髪１: f38 f40 f42 f44 f46 f48 f50 (MMM: 17 17 16 15 15 16 16)');
for (const scale of [1, 10, 20, 25, 33, 50, 66, 100, 150, 200, 300, 500]) {
  await run(`solver50+stiff/${scale}+damp0.05`, { solverIterations: 50, stiffScale: scale, damping: 0.05 });
}
