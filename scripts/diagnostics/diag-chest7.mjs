// 诊断15：轻刚体碰撞禁用 vs solver 迭代提高——胸部偶发爆炸修复
// 参数：mode=nocollide(轻刚体 groupTarget=0) | solver100 | solver150 | solver200
...
const mode = process.argv[2] ?? 'nocollide';
...
const rigidBodyParams = pmx.rigidBodies.map((rb, i) => {
  ...
  if (mode === 'nocollide' && p.type !== 0 && p.weight < 0.1) {
    p.groupTarget = 0; // 轻刚体不参与碰撞
  }
  return p;
});
...
let iters = 50;
if (mode === 'solver100') iters = 100;
if (mode === 'solver150') iters = 150;
if (mode === 'solver200') iters = 200;
try { ph.world.getSolverInfo().set_m_numIterations(iters); } catch (e) {}
...
console.log(`mode=${mode}(iters=${iters})  左胸上: ${fmt(angs.left)}  右胸上: ${fmt(angs.right)}  前髪: ${fmt(angs.mae)}`);
