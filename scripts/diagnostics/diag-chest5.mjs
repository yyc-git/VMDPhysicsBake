// 诊断13：禁用 spring（只留 rotLim）vs 保留 spring——胸部摆动对比
...
const mode = process.argv[2] ?? 'nospring'; // nospring | spring
...
// 保留 spring 时用 scale=2000；禁用 spring 时 patch 成 no-op
if (mode === 'nospring') {
  ...
  Ammo.btGeneric6DofSpringConstraint.prototype.setStiffness = function (idx, val) {
    // 完全禁用 spring（不设置 stiffness，等同无 spring）
    return this;
  };
  // 同时禁 enableSpring？MMDPhysics 里 enableSpring 也会调，patch 掉
  const _origEnable = Ammo.btGeneric6DofSpringConstraint.prototype.enableSpring;
  Ammo.btGeneric6DofSpringConstraint.prototype.enableSpring = function (idx, on) {
    return _origEnable.call(this, idx, false);
  };
} else {
  const _orig = Ammo.btGeneric6DofSpringConstraint.prototype.setStiffness;
  Ammo.btGeneric6DofSpringConstraint.prototype.setStiffness = function (idx, val) {
...
console.log(`mode=${mode}  左胸上: ${fmt(angs.left)}  右胸上: ${fmt(angs.right)}  前髪: ${fmt(angs.mae)}`);
